...
onMounted(() => {
  ...
  function create() {
    ...
    /** Player's position. */
    this.add.text(globalx - 600, globaly, "Player left", {
      fontFamily: 'Inter, "sans-serif"',
    }); /*Player left*/

    this.add.text(globalx + 575, globaly, "Player right", {
      fontFamily: 'Inter, "sans-serif"',
    }); /*Player right*/

    this.add.text(globalx - 20, globaly + 320, "Player down", {
      fontFamily: 'Inter, "sans-serif"',
    }); /*Player down*/

    this.add.text(globalx - 5, globaly - 350, "Player Up", {
      fontFamily: 'Inter, "sans-serif"',
    }); /*Player up*/
    ...
    const TextNameOfRoom = this.add.text(10, 10, "Nom sala: " + nameRoom, {
      fontFamily: 'Inter, "sans-serif"',
    }); /* Name of the room */

    const CurrentPlayer = "Player 5";

    const TurnPlayerName = this.add.text(10, 40, "El turn es del: " + CurrentPlayer, {
      fontFamily: 'Inter, "sans-serif"',
    });
    ...
    props.socket.on("game:rooms:get-cards", (cardsActualPlayer) => {
      ...
      for (let j = 0; j < cardsActualPlayer.length; j++) {
        cardsRecieved[cardsActualPlayer[j]] = this.add
          .image(globalx / 1.26 + j * 30, globaly + 250, cardsActualPlayer[j])
          .setScale(0.35, 0.325)
          .setInteractive();
      }
      if (debug) {
    ...
    this.input.on("pointerover", function (_pointer, objectsClicked) {
      cardsRecieved[objectsClicked[0].texture.key].setScale(0.5, 0.5).setY(550);
